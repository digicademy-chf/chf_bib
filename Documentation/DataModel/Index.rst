...
``Extent`` may be used several times in a bibliographic entry to provide, for
example, volume and number of the journal an article is published in. The same
class is used to enable providing multiple identifiers. ``Agent`` is its
own class because agents may occur multiple times per publication, and their
